...
Em plataformas não Windows, pode executar o Azure PowerShell no PowerShell Core v6. Esta versão do PowerShell é criada para utilizar em qualquer plataforma que suporte .NET Core. Para trabalhar com estas plataformas, existe uma versão especial disponível do .NET Core do Azure PowerShell.
...
As instruções de instalação do PowerShell Core são diferentes para a maioria das distribuições macOS e Linux.
...
O PowerShell Core inclui o módulo PowerShellGet instalado. A instalação de módulos no PowerShell exige privilégios elevados, pelo que irá precisar de iniciar a sua sessão como superutilizador:
...
> [!IMPORTANT]
> O módulo `AzureRM` detalhado noutros artigos não é criado para .NET Core e não irá trabalhar com o PowerShell Core. `AzureRM` e `AzureRM.NetCore` utilizam os mesmos nomes de cmdlet, pelo que a única diferença é o nome do módulo rollup e a versão .NET em que são criados.
...
Irá precisar de repetir estes passos para cada nova sessão do PowerShell iniciada. Importar automaticamente o módulo `AzureRM` exige a configuração de um perfil do PowerShell, pode saber mais sobre isto em [Sobre os Perfis](/powershell/module/microsoft.powershell.core/about/about_profiles).
...
Os módulos do Azure PowerShell para .NET Core ainda estão em desenvolvimento. Estes módulos não fornecem o conjunto completo de cmdlets que estão disponíveis para a versão do Windows dos módulos. As seguintes funções estão implementadas nos módulos AzureRM.Netcore:
