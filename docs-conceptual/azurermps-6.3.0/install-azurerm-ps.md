...
O módulo `AzureRM` é um módulo de rollup para os cmdlets do Azure PowerShell. A instalação faz o download de todos os módulos disponíveis do Azure Resource Manager e disponibiliza os seus cmdlets para utilização.
...
Pode atualizar a sua instalação do Azure PowerShell ao executar o [Update-Module](/powershell/module/powershellget/update-module). Este comando __não__ desinstala as versões anteriores.
...
Para começar a utilizar o Azure PowerShell, veja [Introdução ao Azure PowerShell](get-started-azureps.md) para saber mais sobre o módulo e os seus recursos.