...
* Foram formatadas as mensagens das futuras alterações interruptivas
* Atualização do Azure.Core para 1.4.1

#### <a name="azaks"></a>Az.Aks
* Foi adicionada lógica de validação de parâmetros do lado do cliente para "New-AzAksCluster", "Set-AzAksCluster" e "New-AzAksNodePool". [#12372]
* Foi adicionado suporte para suplementos em "New-AzAksCluster". [#11239]
* Foram adicionados os cmdlets "Enable-AzAksAddOn" e "Disable-AzAksAddOn" para suplementos. [#11239]
* Foi adicionado o parâmetro "GenerateSshKey" para "New-AzAksCluster". [#12371]
* A versão da API foi atualizada para a 2020-06-01.

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* Foram apresentados termos legais adicionais para determinadas APIs.

#### <a name="azcompute"></a>Az.Compute
* Foi adicionado o parâmetro opcional "-EncryptionType" a "New-AzVmDiskEncryptionSetConfig"
* Novos cmdlets para o novo tipo de recurso: DiskAccess "Get-AzDiskAccess", "New-AzDiskAccess", "Get-AzDiskAccess"
* Foram adicionados os parâmetros opcionais "-DiskAccessId" e "-NetworkAccessPolicy" a "New-AzSnapshotConfig"
* Foram adicionados os parâmetros opcionais "-DiskAccessId" e "-NetworkAccessPolicy" a "New-AzDiskConfig"
* Foi adicionada a propriedade "PatchStatus" à Vista de Instância de VirtualMachine
* Foi adicionada a propriedade "VMHealth" à vista de instância da máquina virtual, que é o objeto devolvido quando "Get-AzVm" é invocado com "-Status"
* Foi adicionado o campo "AssignedHost" às vistas de instância "Get-AzVM" e "Get-AzVmss". O campo mostra o ID de recurso da instância da máquina virtual
* Foi adicionado o parâmetro opcional "-SupportAutomaticPlacement" a "New-AzHostGroup" 
* Foi adicionado o parâmetro "-HostGroupId" a "New-AzVm" e "New-AzVmss"

#### <a name="azdatafactory"></a>Az.DataFactory
* Atualização da versão do SDK de .Net do ADF para 4.11.0

#### <a name="azeventhub"></a>Az.EventHub
* Foram adicionados novos cmdlets de Cluster - "New-AzEventHubCluster", "Set-AzEventHubCluster", "Get-AzEventHubCluster", "Remove-AzEventHubCluster", "Get-AzEventHubClustersAvailableRegions".
* Foi corrigido o problema #10722: Correção para atribuir apenas "Listen" aos direitos de AuthorizationRule.

#### <a name="azfunctions"></a>Az.Functions
* Foi removida a capacidade de criar Funções V2 em regiões que não as suportam.
* PowerShell 6.2 preterido. Foi adicionado um aviso para quando um utilizador criar uma aplicação de funções do PowerShell 6.2 que recomenda a criação de uma aplicação de funções do PowerShell 7.0.

#### <a name="azhdinsight"></a>Az.HDInsight
* Criação de cluster com configuração de Dimensionamento Automático suportada
    - Foi adicionado o novo parâmetro "AutoscaleConfiguration" ao cmdlet "New-AzHDInsightCluster"
* Configuração de Dimensionamento Automático do cluster em funcionamento suportada
    - Foi adicionado o novo cmdlet "Get-AzHDInsihgtClusterAutoscaleConfiguration"
    - Foi adicionado o novo cmdlet "New-AzHDInsihgtClusterAutoscaleConfiguration"
    - Foi adicionado o novo cmdlet "Set-AzHDInsihgtClusterAutoscaleConfiguration"
    - Foi adicionado o novo cmdlet "Remove-AzHDInsihgtClusterAutoscaleConfiguration"
    - Foi adicionado o novo cmdlet "New-AzHDInsihgtClusterAutoscaleScheduleCondition"

#### <a name="azkeyvault"></a>Az.KeyVault
* Foi adicionado o suporte da autorização RBAC [#10557]
* Processamento de erros melhorado em "Set-AzKeyVaultAccessPolicy" [#4007]

#### <a name="azkusto"></a>Az.Kusto
* Disponibilidade geral do módulo "Az.Kusto"

#### <a name="aznetwork"></a>Az.Network
* [Alteração interruptiva] Os cmdlets abaixo foram atualizados para estarem alinhados com o router e o hub virtuais de recursos
    - "New-AzVirtualRouter": 
        - Foi adicionado o parâmetro -HostedSubnet para suportar o recurso subordinado da configuração de IP
        - -HostedGateway e -HostedGatewayId foram eliminados
    - "Get-AzVirtualRouter":
        - Foi adicionado um conjunto de parâmetros ao nível da subscrição
    - "Remove-AzVirtualRouter"
    - "Add-AzVirtualRouterPeer"
    - "Get-AzVirtualRouterPeer"
    - "Remove-AzVirtualRouterPeer"
* Foi adicionado um novo cmdlet para a Porta do Azure Express Route
    - "New-AzExpressRoutePortLOA"
* Foi adicionada a propriedade RemoteBgpCommunities ao Recurso de Peering de VirtualNetwork
* Foi modificada a mensagem de aviso de "New-AzLoadBalancerFrontendIpConfig", "New-AzPublicIpAddress" e "New-AzPublicIpPrefix".
* Foi adicionado VpnGatewayIpConfigurations à saída "Get-AzVpnGateway"
* Foi corrigido o erro de "Set-AzApplicationGatewaySslCertificate" [#9488]
* Foi adicionado o parâmetro "AllowActiveFTP" a "AzureFirewall"
* Foram atualizados os seguintes comandos para a funcionalidade: Ativação da definição/remoção de segurança na Internet no P2SVpnGateway de VirtualWan.
- Atualização de "New-AzP2sVpnGateway": Foi adicionado o parâmetro opcional "EnableInternetSecurityFlag" para que os clientes definam como true para ativar a segurança na Internet no P2SVpnGateway, que será aplicado aos clientes de Ponto a site.
- Atualização de "Update-AzP2sVpnGateway": Foi adicionado o parâmetro opcional "EnableInternetSecurityFlag" ou "DisableInternetSecurityFlag" para que os clientes definam como true/false para ativar/desativar a segurança na Internet no P2SVpnGateway, que será aplicado aos clientes de Ponto a site.
* Foi adicionado o novo cmdlet "Reset-AzP2sVpnGateway" para que os clientes redefinam/reiniciem o P2SVpnGateway de VirtualWan para resolução de problemas.
* Foi adicionado o novo cmdlet "Reset-AzVpnGateway" para que os clientes redefinam/reiniciem o VpnGateway de VirtualWan para resolução de problemas.
* Atualização de "Set-AzVirtualNetworkSubnetConfig"
    - Definir as propriedades do NSG e da Tabela de Encaminhamento da sub-rede como null se estiverem definidas explicitamente nos parâmetros [#1548][#9718]

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Foi corrigido o Estado de Eliminação para Itens de Cópia de Segurança das cargas de trabalho.

#### <a name="azresources"></a>Az.Resources
* Foi adicionada uma verificação em falta para Set-AzRoleAssignment
* Foi adicionado o atributo de alteração interruptiva ao parâmetro "SubscriptionId" de "Get-AzResourceGroupDeploymentOperation"
* Foram atualizados os cmdlets What-If do modelo do ARM para mostrar as alterações de recursos "Ignore" no final
* Foram corrigidos os problemas de serialização de parâmetros seguros e de matriz para cmdlets de implementação [#12773]

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Foram adicionados novos cmdlets aos tipos de nós e clusters geridos:
...
* Atualização do Service Fabric SDK para a versão 1.2.0, que utiliza a versão de API do fornecedor de serviços do Service Fabric 2020-03-01 para o modelo atual e 2020-01-01-preview para clusters geridos.

#### <a name="azsql"></a>Az.Sql
* Foi adicionado BackupStorageRedundancy a "New-AzSqlInstance" e "Get-AzSqlInstance"
* Foi adicionado o cmdlet "Get-AzSqlServerActiveDirectoryOnlyAuthentication"
* Foi adicionado o cmdlet "Enable-AzSqlServerActiveDirectoryOnlyAuthentication"
* Foi adicionado o parâmetro Force a "New-AzSqlInstance"
* Foram adicionados cmdlets para o serviço Managed Database Log Replay
    - "Start-AzSqlInstanceDatabaseLogReplay"
    - "Get-AzSqlInstanceDatabaseLogReplay"
    - "Complete-AzSqlInstanceDatabaseLogReplay"
    - "Stop-AzSqlInstanceDatabaseLogReplay"
* Foi adicionado o cmdlet "Get-AzSqlInstanceActiveDirectoryOnlyAuthentication"
* Foi adicionado o cmdlet "Enable-AzSqlInstanceActiveDirectoryOnlyAuthentication"
* Foi adicionado o cmdlet "Disable-AzSqlInstanceActiveDirectoryOnlyAuthentication"
* Foram adicionados os cmdlets "New-AzSqlDatabaseImport" e "New-AzSqlDatabaseExport" para suportar a funcionalidade de isolamento de rede
* Foi adicionado o cmdlet "New-AzSqlDatabaseImportExisting"
* Foram atualizados os cmdlets das Bases de Dados para suportar a especificação do tipo de armazenamento de cópias de segurança
* Foi adicionado o parâmetro Force a "New-AzSqlDatabase"
* Foi adicionado um aviso para a configuração de BackupStorageRedundancy nas regiões selecionadas em "New-AzSqlDatabase"
* Foram atualizados os cmdlets ActiveDirectoryOnlyAuthentication para o servidor e a instância para incluir ResourceId e InputObject

#### <a name="azstorage"></a>Az.Storage
* Foi corrigida a falha no blob de carregamento através da atualização de Microsoft.Azure.Storage.DataMovement 2.0.0 [#12220]
* Restauro para um Ponto Anterior no Tempo suportado
    - "Enable-AzStorageBlobRestorePolicy"
    - "Disable-AzStorageBlobRestorePolicy"
    - "New-AzStorageBlobRangeToRestore"
    - "Restore-AzStorageBlobRange"
* Suporte para obter o estado de restauro de blobs da conta de armazenamento ao executar get-AzureRMStorageAccount com o parâmetro -IncludeBlobRestoreStatus 
    - "Get-AzureRMStorageAccount"
* Foi adicionada uma mensagem de aviso de alteração interruptiva para alteração da saída dos cmdlets futuros
...
* O SDK Microsoft.Azure.Cosmos.Table foi atualizado para 1.0.8

### <a name="thanks-to-our-community-contributors"></a>Agradecemos aos colaboradores da comunidade
...
* Ravi Mishra (@inmishrar), atualização da pilha de runtime da Aplicação Web do Azure para DOTNETCORE (#12833) 
* @jack-education, Set-AzVirtualNetworkSubnetConfig atualizado para permitir a remoção do NSG e da Tabela de Encaminhamento da sub-rede (#12351) 
...
* Joshua Van Daalen (@greenSacrifice)
  ...
  * Atualização dos exemplos de New-AzResourceLock.md (#12806)
* Eragon Riddle (@eragonriddle), correção do nome do campo do parâmetro no exemplo (#12825) 
* @rossifumax, correção do erro de digitação em New-AzConfigurationAssignment.md (#12701)

## <a name="461---august-2020"></a>4.6.1 - Agosto de 2020
#### <a name="azcompute"></a>Az.Compute
* Correção do parâmetro "-EncryptionAtHost" em "New-AzVm" para remover o valor predefinido de falso [#12776]

## <a name="460---august-2020"></a>4.6.0 - Agosto de 2020
#### <a name="azaccounts"></a>Az.Accounts
* São carregados todos os ambientes de cloud pública quando o ponto final de deteção não devolve o AzureCloud predefinido ou outros ambientes públicos [#12633]
* Exposição de SubscriptionPolicies em "Get-AzSubscription" [#12551]

#### <a name="azautomation"></a>Az.Automation
* Adição de parâmetros "-RunOn" a "Set-AzAutomationWebhook" para especificar um Grupo de Função de Trabalho Híbrida
...
* Adição do comutador "-InstanceView" como parâmetro opcional a "Get-AzHostGroup"
* Adição do novo cmdlet "Invoke-AzVmPatchAssessment"

#### <a name="azdatafactory"></a>Az.DataFactory
* Adição das propriedades em falta à classe PSPipelineRun.

#### <a name="azhdinsight"></a>Az.HDInsight
* É suportada a criação de clusters com encriptação na funcionalidade de anfitrião.

#### <a name="azkeyvault"></a>Az.KeyVault
* Adição de mensagens de aviso para planear a desativação da eliminação recuperável
* Adição de mensagens de aviso para planear a remoção do atributo SecretValueText

#### <a name="azmaintenance"></a>Az.Maintenance
* Adição de campos relacionados com agendamento opcionais a "New-AzMaintenanceConfiguration"
* Adição do novo cmdlet para "Get-AzMaintenancePublicConfiguration"

#### <a name="azmanagedservices"></a>Az.ManagedServices
* Adição de avisos de alteração interruptiva nos cmdlets de atribuição e definição de serviços geridos

#### <a name="azmonitor"></a>Az.Monitor
* Expansão do parâmetro definido em "Set-AzDiagnosticSetting" para a separação da ativação de Registos e Métricas [#12482]
* Correção do erro de "Add-AzMetricAlertRuleV2" ao obter o alerta de métrica do pipeline

#### <a name="azresources"></a>Az.Resources
* Atualização da resposta "Get-AzPolicyAlias" no sentido de incluir informações que indicam se o alias pode ser modificado pelo Azure Policy.
* Criação do novo cmdlet "Set-AzRoleAssignment"
* Adição de "Get-AzDeploymentManagementGroupWhatIfResult" para obter os resultados de Hipóteses do modelo do ARM no âmbito do Grupo de gestão
* Adição do novo cmdlet "Get-AzTenantWhatIfResult" para obter os resultados de Hipóteses do modelo do ARM no âmbito do inquilino
* Substituição de "-WhatIf" e "-Confirm" por "New-AzManagementGroupDeployment" e "New-AzTenantDeployment" no sentido de utilizar os resultados de Hipóteses do modelo do ARM
* Correção dos comportamentos de "-WhatIf" e "-Confirm" para os novos cmdlets de implementação para que estejam em conformidade com Falso e 
* Correção do erro de serialização de "-TemplateObject" e "TemplateParameterObject" [#1528] [#6292]
* Adição do atributo de alteração interruptiva a "Get-AzResourceGroupDeploymentOperation" para a próxima alteração do tipo de saída

#### <a name="azsignalr"></a>Az.SignalR
* Correção dos erros dos ficheiros de ajuda "Restart-AzSignalR" e "Update-AzSignalR"
* Adição dos cmdlets "Update-AzSignalRNetworkAcl" e "Set-AzSignalRUpstream"

#### <a name="azstorage"></a>Az.Storage
* É suportada a aceleração da consulta de blobs
    -  "Get-AzStorageBlobQueryResult"
    -  "New-AzStorageBlobQueryConfig"
* Atualização do ficheiro de ajuda, com a adição de mais descrições e a correção de erros de digitação
    -  "Start-AzStorageBlobCopy"
    -  "Get-AzDataLakeGen2Item"
* Correção da falha de transferência de blobs quando o subdiretório relacionado não existe [#12592]
    -  "Get-AzStorageBlobContent"
* É suportada a Política de Replicação Definir/Obter/Remover Objeto nas Contas de armazenamento
    - "New-AzStorageObjectReplicationPolicyRule"
    - "Set-AzStorageObjectReplicationPolicy"
    - "Get-AzStorageObjectReplicationPolicy"
    - "Remove-AzStorageObjectReplicationPolicy"
* Suporte para ativar/desativar o ChangeFeed no Serviço Blob de uma Conta de armazenamento
    - "Update-AzStorageBlobServiceProperty"

## <a name="450---august-2020"></a>4.5.0 - Agosto de 2020
#### <a name="azaccounts"></a>Az.Accounts
* "Connect-AzAccount" atualizado para aceitar o parâmetro "MaxContextPopulation" [#9865]
* Versão de SubscriptionClient atualizada para a de 2019-06-01 e apresentação de domínios de inquilino [#9838]
* Inquilino principal suportado e informações do inquilino managedBy da subscrição
* Nome do módulo corrigido, informações da versão nos dados telemétricos
* SqlDatabaseDnsSuffix e ServiceManagementUrl ajustados se o ponto final dos metadados do ambiente devolverem valores incompatíveis

#### <a name="azaks"></a>Az.Aks
* "ClientIdAndSecret" removido para "ServicePrincipalIdAndSecret" e "ClientIdAndSecret" definido como um alias [#12381].
* "Get-AzAks"/"New-AzAks"/"Remove-AzAks"/"Set-AzAks" removidos para "Get-AzAksCluster"/"New-AzAksCluster"/"Remove-AzAksCluster"/"Set-AzAksCluster" e definição dos valores originais como alias [#12373].

#### <a name="azapimanagement"></a>Az.ApiManagement
* Novo cmdlet "Add-AzApiManagementApiToGateway" adicionado.
* Novo cmdlet "Get-AzApiManagementGateway" adicionado.
* Novo cmdlet "Get-AzApiManagementGatewayHostnameConfiguration" adicionado.
* Novo cmdlet "Get-AzApiManagementGatewayKey" adicionado.
* Novo cmdlet "New-AzApiManagementGateway" adicionado.
* Novo cmdlet "New-AzApiManagementGatewayHostnameConfiguration" adicionado.
* Novo cmdlet "New-AzApiManagementResourceLocationObject" adicionado.
* Novo cmdlet "Remove-AzApiManagementApiFromGateway" adicionado.
* Novo cmdlet "Remove-AzApiManagementGateway" adicionado.
* Novo cmdlet "Remove-AzApiManagementGatewayHostnameConfiguration" adicionado.
* Novo cmdlet "Update-AzApiManagementGateway" adicionado.
* Foi adicionado um novo parâmetro [-GatewayId] opcional ao cmdlet "Get-AzApiManagementApi".

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* Utilizado "Deny" especificamente como ação NetworkRules predefinida.

#### <a name="azfrontdoor"></a>Az.FrontDoor
* Foi resolvido um problema em que é gerada uma exceção quando Enum.Parse tenta limitar um valor nulo a um valor de enumeração Enabled ou Disabled [#12344]

#### <a name="azhdinsight"></a>Az.HDInsight
* É suportada a criação de clusters com encriptação na funcionalidade de trânsito.
    - Foi adicionado o novo parâmetro "EncryptionInTransit" ao cmdlet "New-AzHDInsightCluster"
    - Foi adicionado o novo parâmetro "EncryptionInTransit" ao cmdlet "New-AzHDInsightClusterConfig"
* É suportada a criação de clusters com a funcionalidade de ligação privada:
    - Novos parâmetros "PublicNetworkAccessType" e "OutboundPublicNetworkAccessType" adicionados ao cmdlet "New-AzHDInsightCluster"
    - Novos parâmetros "PublicNetworkAccessType" e "OutboundPublicNetworkAccessType" adicionados ao cmdlet "New-AzHDInsightClusterConfig"
* Informações de rede virtual devolvidas ao chamar "New-AzHDInsightCluster" ou "Get-AzHDInsightCluster"

#### <a name="aznetwork"></a>Az.Network
* Adição de suporte para o parâmetro AddressPrefixType a "Remove-AzExpressRouteCircuitConnectionConfig"
* Alterações não interruptivas adicionadas: Funcionalidade PeerAddressType para Peering Privado em "Remove-AzExpressRouteCircutPeeringConfig".
* Código alterado para ignorar caso dos parâmetros AddressPrefixType e PeerAddressType.
* Foi modificada a mensagem de aviso de "New-AzLoadBalancerFrontendIpConfig", "New-AzPublicIpAddress" e "New-AzPublicIpPrefix".

#### <a name="azoperationalinsights"></a>Az.OperationalInsights
* Opção "-ForceDelete" adicionada para "Remove-AzOperationalInsightsworkspace"
* Novo cmdlet "Get-AzOperationalInsightsDeletedWorkspace" adicionado
* Novo cmdlet "Restore-AzOperationalInsightsWorkspace" adicionado

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Experiência de deteção de contentores/itens do Azure Backup melhorada.

#### <a name="azresources"></a>Az.Resources
* Propriedades "Condition", "ConditionVersion" e "Description" adicionadas a "New-AzRoleAssignment"
    - Isto incluiu todas as alterações relevantes aos modelos de dados

#### <a name="azsql"></a>Az.Sql
* Foi corrigido o potencial erro de nome do servidor não sensível a maiúsculas e minúsculas em "New-AzSqlServer" e "Set-AzSqlServer"
* Foi corrigido o erro em que era devolvido um nome de base de dados incorreto numa base de dados existente em "New-AzSqlDatabaseSecondary"

#### <a name="azstorage"></a>Az.Storage
* Criação de tokens de SAS de contentores/blobs suportada com nova permissão x,t
    -  "New-AzStorageBlobSASToken"
    -  "New-AzStorageContainerSASToken"
* Criação de tokens de SAS de contas suportada com nova permissão x,t,f
    -  "New-AzStorageAccountSASToken"
* Obtenção da utilização de partilha de ficheiros única suportada
    - "Get-AzRmStorageShare"

## <a name="440---july-2020"></a>4.4.0 - Julho de 2020
#### <a name="azaccounts"></a>Az.Accounts
* Adição do novo cmdlet "Invoke-AzRestMethod"
* Correção de um problema que pode causar erros de autenticação em cenários de vários processos, tais como executar vários cmdlets do Azure PowerShell utilizando "Start-Job" [#9448]

#### <a name="azaks"></a>Az.Aks
* Correção do erro "Get-AzAks" uma vez que não recebe todos os clusters [#12296]

#### <a name="azanalysisservices"></a>Az.AnalysisServices
* Remoção da referência do projeto para Autenticação

#### <a name="azautomation"></a>Az.Automation
* Correção do problema em que a cadeia com chars de fuga não pode ser convertida num objeto json.

#### <a name="azcompute"></a>Az.Compute
* Adição do aviso ao utilizar "New-AzVmss" sem a versão de imagem "mais recente"

#### <a name="azdatafactory"></a>Az.DataFactory
* Adição de parâmetros globais ao Data Factory.

#### <a name="azeventgrid"></a>Az.EventGrid
* Atualização para utilizar a versão de API 2020-06-01.
* Novas funcionalidades adicionadas:
    - Mapeamento de entrada
    - Esquema de Entrega de Eventos
    - Ligação Privada
    - Esquema de Eventos na Cloud V10
    - Tópico do Service Bus como Destino
    - Função do Azure como Destino
    - Criação de batches do WebHook
    - Webhook seguro (suporte de AAD)
    - IpFiltering
* cmdlets atualizados:
    - "New-AzEventGridSubscription"/"Update-AzEventGridSubscription":
        - Adicionar novos parâmetros opcionais para suportar a criação de batches do webhook.
        - Adicionar novos parâmetros opcionais para suportar o webhook seguro através do AAD.
        - Adicionar uma nova enumeração para o parâmetro EndpointType para suportar a função do Azure e o tópico do Service Bus enquanto novos destinos.
        - Adicionar novo parâmetro opcional para o esquema de entrega.
    - "New-AzEventGridTopic"/"Update-AzEventGridTopic" e "New-AzEventGridDomain"/"Update-AzEventGridDomain":
        - Adicionar novos parâmetros opcionais para suportar IpFiltering.
    - "New-AzEventGridTopic"/"New-AzEventGridDomain":
        - Adicionar novos parâmetros opcionais para suportar o Mapeamento de entrada.

#### <a name="azfrontdoor"></a>Az.FrontDoor
* Módulo atualizado para utilizar a API 2020-05-01
* Adição de suporte do Private Link para recursos de Armazenamento, Cofre de chaves e Serviço de Aplicações Web

#### <a name="azhdinsight"></a>Az.HDInsight
* Suporte para a criação de cluster com armazenamento ADLSGen1/2 em clouds nacionais.

#### <a name="azmonitor"></a>Az.Monitor
* Correção do erro em "Get-AzDiagnosticSetting" quando as métricas ou registos são nulos [#12272]

#### <a name="aznetwork"></a>Az.Network
* Correção da troca de parâmetros na ligação VWan HubVnet
* Adição de novos cmdlets para Sites de Aplicações Virtuais da Rede Azure
    - "Get-AzVirtualApplianceSite"
    - "New-AzVirtualApplianceSite"
    - "Remove-AzVirtualApplianceSite"
    - "Update-AzVirtualApplianceSite"
    - "New-AzOffice365PolicyProperty"
* Adição de novos cmdlets para Aplicações Virtuais da Rede Azure
...
* Gateway de Aplicação integrado para Cmdlets Comuns do Private Link
* StorageSync integrado para Cmdlets Comuns do Private Link
* SignalR integrado para Cmdlets Comuns do Private Link

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Remoção da referência do projeto para Autenticação
* Os cmdlets afinados do Azure Backup ajudam o texto a ser mais preciso.
* O Azure Backup adicionou suporte para obter tarefas de agentes MAB através do cmdlet "Get-AzRecoveryServicesBackupJob".

#### <a name="azresources"></a>Az.Resources
* Atualização de "Save-AzResourceGroupDeploymentTemplate" para utilizar o SDK.
* Adição do cmdlet "Unregister-AzResourceProvider".

#### <a name="azsql"></a>Az.Sql
* Adição de suporte para o Principal de serviço e utilizadores convidados no cmdlet Set-AzSqlInstanceActiveDirectoryAdministrator
* Correção de um erro em cmdlets de Classificação de Dados.
* Adição de suporte para a ativação pós-falha do Azure SQL Managed Instance: "Invoke-AzSqlInstanceFailover"

#### <a name="azstorage"></a>Az.Storage
* Correção do erro de que o UserAgent não é adicionado para alguns cmdlets do plano de dados.
* Suporte para a criação/atualização da Conta de armazenamento com MinimumTlsVersion e AllowBlobPublicAccess
    -  "New-AzStorageAccount"
    -  "Set-AzStorageAccount"
* Suporte para Permitir/desativar o controlo de versões no Serviço Blob de uma Conta de armazenamento
    - "Update-AzStorageBlobServiceProperty"
* Suporte para a lista de blobs com versões de blobs
    - "Get-AzStorageBlob"
* Suporte para obter/remover instantâneo de blob único ou versão do blob
    - "Get-AzStorageBlob"
    - "Remove-AzStorageBlob"
* Suporte para pipeline a partir do objeto de blob gerado a partir de Azure.Storage.Blobs V12
    - "Get-AzStorageBlobContent"
    - "New-AzStorageBlobSASToken"
    - "Remove-AzStorageBlob"
...
* Adição de uma nova versão StorageSync SDK para ApiVersion 2020-03-01
* Adição de cmdlet do Serviço de Sincronização de Armazenamento de Atualização
    - "Set-AzStorageSyncService"
* Adição de IncomingTrafficPolicy e PrivateEndpointConnections para cmdlets StorageSyncService.

#### <a name="azwebsites"></a>Az.Websites
* Adição de suporte para a realização de operações para Slots em grupos de recursos diferentes do Plano do Serviço de Aplicações

## <a name="430---june-2020"></a>4.3.0 - Junho de 2020
#### <a name="azaccounts"></a>Az.Accounts
* Suporte para a deteção da definição do ambiente por predefinição e adição de ambiente através de "Add-AzEnvironment"
* Atualizar assemblagens pré-carregadas [#12024], [#11976]
* Atualização da assemblagem Azure.Core
* Correção de um problema que pode provocar a falha de "Connect-AzAccount" numa execução de múltiplos threads [#11201]

#### <a name="azaks"></a>Az.Aks
* Substituição da utilização da [API AccessProfile](https://docs.microsoft.com/rest/api/aks/managedclusters/getaccessprofile) antiga por chamadas para as APIs [ListClusterAdmin](https://docs.microsoft.com/rest/api/aks/managedclusters/listclusteradmincredentials) e [ListClusterUser](https://docs.microsoft.com/rest/api/aks/managedclusters/listclusterusercredentials)

#### <a name="azbatch"></a>Az.Batch
* Atualização da versão do SDK de Az.Batch para utilizar "Microsoft.Azure.Management.Batch" para 11.0.0
* Adição da capacidade de definir a Identidade BatchAccount no cmdlet "New-AzBatchAccount"

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* Suporte para a apresentação das capacidades da conta.
* Suporte para a modificação de PublicNetworkAccess.

#### <a name="azcompute"></a>Az.Compute
* Adição do parâmetro SimulateEviction aos cmdlets Set-AzVM e Set-AzVmssVM.
* Adição de "Premium_LRS" ao mecanismo de preenchimento de argumentos do parâmetro StorageAccountType para o cmdlet New-AzGalleryImageVersion.
* Adição de subestados a VMCustomScriptExtension [#11297]
* Adição de "Delete" ao mecanismo de preenchimento de argumentos do parâmetro EvictionPolicy para os cmdlets New-AzVM e New-AzVMConfig.
* Correção do nome da nova Extensão de VM para SAP

#### <a name="azdatafactory"></a>Az.DataFactory
* Atualização da versão do SDK de .Net do ADF para 4.9.0

#### <a name="azeventhub"></a>Az.EventHub
* Adição de parâmetros de Identidade Gerida aos cmdlets "New-AzEventHubNamespace" e "Set-AzEventHubNamespace"

#### <a name="azfunctions"></a>Az.Functions
* Adição de suporte para a criação de aplicações de funções do PowerShell 7.0 e de Java 11

#### <a name="azhdinsight"></a>Az.HDInsight
* Suporte para listagem de anfitriões e reinício de anfitriões específicos do cluster HDInsight.

#### <a name="azhealthcareapis"></a>Az.HealthcareApis
* Atualização da versão do SDK para 1.1.0
* Adição de suporte para as definições de Exportação e a Identidade Gerida

#### <a name="azmonitor"></a>Az.Monitor
* Correção do parâmetro de objeto de entrada para "Set-AzActivityLogAlert"
* Correção do parâmetro "InputObject" para "Set-AzActionGroup" [#10868]

#### <a name="aznetwork"></a>Az.Network
* Adição de suporte para o parâmetro AddressPrefixType a "Remove-AzExpressRouteCircuitConnectionConfig"
* Adição de novos cmdlets para a Política de Firewall do Azure
    - "New-AzFirewallPolicyDnsSetting"
    - Suporte para FQDN de Destino nas Regras de Rede para a Política de Firewall
* Adição de suporte para operações de conjuntos de endereços back-end
    - "New-AzLoadBalancerBackendAddressConfig"
    - "New-AzLoadBalancerBackendAddressPool"
    - "Set-AzLoadBalancerBackendAddressPool"
    - "Remove-AzLoadBalancerBackendAddressPool"
    - "Get-AzLoadBalancerBackendAddressPool"
* Adição de validação de nomes para "New-AzIpGroup"
* Adição de novos cmdlets para a Política de Firewall do Azure
    - "New-AzFirewallPolicyThreatIntelWhitelist"
* Foram atualizados os seguintes comandos para a funcionalidade: Definição/remoção de servidores DNS personalizados no P2SVpnGateway de VirtualWan.
    - Atualização de New-AzP2sVpnGateway: Adição do parâmetro opcional "-CustomDnsServer" para os clientes especificarem os respetivos servidores DNS a definir no P2SVpnGateway, o qual pode ser utilizado por clientes do tipo Ponto a site.
    - Atualização de Update-AzP2sVpnGateway: Adição do parâmetro opcional "-CustomDnsServer" para os clientes especificarem os respetivos servidores DNS a definir no P2SVpnGateway, o qual pode ser utilizado por clientes do tipo Ponto a site.
* Atualização de "Update-AzVpnGateway"
    - Adição do parâmetro opcional "-BgpPeeringAddress" para os clientes especificarem os respetivos bgps personalizados a definir no VpnGateway.
* Adição de novo cmdlet para suportar a reposição do estado de encaminhamento de um recurso VirtualHub:
    - "Reset-AzHubRouter"
* Atualização dos elementos indicados abaixo com base na alteração recente de Swagger para a Política de Firewall
    - Alteração de nomes para RuleGroup, RuleCollectionGroup e RuleType
    - Adição de suporte para Coleções de Regras de NAT da Política de Firewall para suportar várias Coleções de Regras de NAT
* [Alteração Interruptiva] Adição do parâmetro obrigatório "SourceIpGroup" para "New-AzFirewallPolicyApplicationRule" e "New-AzFirewallPolicyNetworkRule".
* [Alteração Interruptiva] Correção do parâmetro "SourceAddress" de "New-AzFirewallPolicyApplicationRule" de modo a ser obrigatório.
* [Alteração Interruptiva] Correção do parâmetro "SourceAddress" de "New-AzFirewallPolicyApplicationRule" de modo a ser obrigatório.
* [Alteração Interruptiva] Remoção de parâmetros obrigatórios: "TranslatedAddress", "TranslatedPort" para "New-AzFirewallPolicyNatRuleCollection".
* Adição de novos cmdlets para suportar PrivateLink no Gateway de Aplicação
...
* Adição de novos cmdlets para o recurso subordinado HubRouteTables de VirtualHub.
    - "New-AzVHubRoute"
    - "New-AzVHubRouteTable"
    - "Get-AzVHubRouteTable"
    - "Update-AzVHubRouteTable"
    - "Remove-AzVHubRouteTable"
* Atualização dos cmdlets existentes para suportar o parâmetro de entrada opcional RoutingConfiguration para encaminhamento personalizado em VirtualWan.
...
#### <a name="azoperationalinsights"></a>Az.OperationalInsights
* Correção do erro PSWorkspace não implementa IOperationalInsightsWorkspace [#12135]
* Adição de "pergb2018" ao conjunto de valores válidos do parâmetro "Sku" em "Set-AzOperationalInsightsWorkspace" 
* Adição do alias "FunctionParameters" para o parâmetro "FunctionParameter" a
    - "New-AzOperationalInsightsSavedSearch"
    - "Set-AzOperationalInsightsSavedSearch"

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Adição de suporte no Azure Backup para a obtenção de itens de MAB.
* Suporte do Azure Site Recovery para o tipo de disco "StandardSSD_LRS"

#### <a name="azresources"></a>Az.Resources
* Adição de "UsageLocation", "GivenName", "Surname", "AccountEnabled", "MailNickname", "Mail" em "PSADUser" [#10526] [#10497]
* Correção do problema em que "-Mail" não funciona em "Get-AzADUser" [#11981]
* Adição do parâmetro "-ExcludeChangeType" a "Get-AzDeploymentWhatIfResult" e "Get-AzResourceGroupDeploymentWhatIfResult"
* Adição do parâmetro "-WhatIfExcludeChangeType" a "New-AzDeployment" e "New-AzResourceGroupDeployment"
* Atualização de cmdlets "Test-Az*Deployment" para apresentação de mensagens de erro melhores
* Correção da mensagem de ajuda para o parâmetro "-Name" de criação de implementação e cmdlets What-If

#### <a name="azsql"></a>Az.Sql
* Adição de suporte para o principal de serviço para o cmdlet Set SQL Server Azure Active Directory Admin
* Correção de problema de sincronização em cmdlets de Classificação de Dados.
* Suporte para a pesquisa de utilizador por correio em "Set-AzSqlServerActiveDirectoryAdministrator" [#12192]

#### <a name="azstorage"></a>Az.Storage
* Suporte para a criação de Conta de armazenamento com RequireInfrastructureEncryption
    -  "New-AzStorageAccount"
* Mudança da lógica de carregamento de Azure.Core para Az.Accounts

#### <a name="azwebsites"></a>Az.Websites
* Adição de salvaguarda para eliminar a aplicação Web criada em caso de falha do restauro em "Restore-AzDeletedWebApp"
* Adição de "SourceWebApp.Location" para "New-AzWebApp" e "New-AzWebAppSlot"
* Correção do erro que impedia a alteração de definições de Contentor em "Set-AzWebApp" e "Set-AzWebAppSlot"
* Correção do erro para obter SiteConfig quando -Name não é fornecido para Get-AzWebApp
* Adição de suporte para a criação de ASP para Aplicações Linux
* Adição de exceções para clonagem entre grupos de recursos

## <a name="420---june-2020"></a>4.2.0 - Junho de 2020
#### <a name="azaccounts"></a>Az.Accounts
* Correção de um problema que fazia com que o Az ignorasse registos nas tarefas da Automatização do Azure ou do PowerShell [#11492]

#### <a name="azanalysisservices"></a>Az.AnalysisServices
* Atualização da versão de assemblagem dos cmdlets de plano de dados

#### <a name="azapimanagement"></a>Az.ApiManagement
* Atualização da versão de assemblagem dos cmdlets de gestão de serviços

#### <a name="azbilling"></a>Az.Billing
* Atualização da versão de assemblagem dos cmdlets de consumo

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* Suporte para o controlo de PrivateEndpoint e PublicNetworkAccess. 

#### <a name="azdatafactory"></a>Az.DataFactory
* Atualização da versão de assemblagem dos cmdlets V2 de fábrica de dados
...
#### <a name="azoperationalinsights"></a>Az.OperationalInsights
* Atualização do SDK para a versão 0.21.0
* Adição de parâmetros opcionais a 
    - "New-AzOperationalInsightsSavedSearch"
    - "Set-AzOperationalInsightsSavedSearch"

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* Correção do exemplo 3 de "Start-AzPolicyComplianceScan"

#### <a name="azpowerbiembedded"></a>Az.PowerBIEmbedded
* Atualização da versão de assemblagem dos cmdlets do PowerBI

#### <a name="azprivatedns"></a>Az.PrivateDns
* Correção da formatação da cadeia de saída verbosa de Remove-AzPrivateDnsRecordSet

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Suporte do Azure Site Recovery para a criação de um plano de recuperação para replicação zona a zona a partir de uma entrada xml.
* Atualização da versão de assemblagem dos cmdlets do SiteRecovery e Backup

#### <a name="azresources"></a>Az.Resources
* Adição do parâmetro Tail aos cmdlets Get-AzDeploymentScriptLog e Save-AzDeploymentScriptLog
* Formatação da propriedade Output e apresentação da mesma na saída do cmdlet Get-AzDeploymentScript
* Mudança de nome do parâmetro -DeploymentScriptInputObject para -DeploymentScriptObject
* Correção do nome em falta do ficheiro/destino nas mensagens de cmdlet.
* Atualização da versão de assemblagem dos cmdlets de gestor de recursos e de etiquetas
...
* Adição do suporte de pesquisa de Utilizador convidado ao cmdlet Set SQL Server Azure Active Directory Admin
...
* Versões do PowerShell suportadas: Windows PowerShell 5.1, PowerShell Core 6.2.4+, PowerShell 7
* Disponibilidade geral de Az.Functions 
* Az.ApiManagement, Az.Batch, Az.Compute, Az.KeyVault, Az.Monitor, Az.Network, Az.OperationalInsights, Az.Resources e Az.Storage são alvo de grandes versões

#### <a name="azaccounts"></a>Az.Accounts
* "Add-AzEnvironment" e "Set-AzEnvironment" foram atualizados para aceitar os parâmetros "AzureSynapseAnalyticsEndpointResourceId" e "AzureSynapseAnalyticsEndpointSuffix"
* Foram adicionadas assemblagens relacionadas ao Azure.Core em Az.Accounts; as plataformas suportadas do PowerShell incluem Windows PowerShell 5.1, PowerShell Core 6.2.4, PowerShell 7+

#### <a name="azaks"></a>Az.Aks
* Versão da API atualizada para 2019-10-01
* Suportado para criar o AKS com contentores do Windows
...
#### <a name="azapimanagement"></a>Az.ApiManagement
* "New-AzApiManagement" e "Set-AzApiManagement": o nome do parâmetro [-AssignIdentity] foi mudado para [-SystemAssignedIdentity]
* "New-AzApiManagement" e "Set-AzApiManagement": Novo parâmetro adicionado: [-UserAssignedIdentity <String[]>]
* "Get-AzApiManagementProperty": o nome foi mudado para "Get-AzApiManagementNamedValue". O nome do parâmetro PropertyId foi mudado para NamedValueId.
* "New-AzApiManagementProperty": o nome foi mudado para "New-AzApiManagementNamedValue". O nome do parâmetro PropertyId foi mudado para NamedValueId. 
* "Set-AzApiManagementProperty": o nome foi mudado para "Set-AzApiManagementNamedValue". O nome do parâmetro PropertyId foi mudado para NamedValueId.
* "Remove-AzApiManagementProperty": o nome foi mudado para "Remove-AzApiManagementNamedValue". O nome do parâmetro PropertyId foi mudado para NamedValueId.
* Foi adicionado o cmdlet novo "Get-AzApiManagementAuthorizationServerClientSecret" e "Get-AzApiManagementAuthorizationServer" vai deixar de devolver o segredo do cliente.
* Foi adicionado o cmdlet novo "Get-AzApiManagementNamedValueSecretValue" e "Get-AzApiManagementNamedValue" vai deixar de devolver o valor do segredo.
* Foi adicionado o cmdlet novo "Get-AzApiManagementOpenIdConnectProviderClientSecret" e "Get-AzApiManagementOpenIdConnectProvider" vai deixar de devolver o segredo do cliente.
* Foi adicionado o cmdlet "Get-AzApiManagementSubscriptionKey" e "Get-AzApiManagementSubscription" vai deixar de devolver chaves de subscrições.
* Foi adicionado o cmdlet novo "Get-AzApiManagementTenantAccessSecret" e "Get-AzApiManagementTenantAccess" vai deixar de devolver chaves.
* Foi adicionado o cmdlet novo "Get-AzApiManagementTenantGitAccessSecret" e "Get-AzApiManagementTenantGitAccess" vai deixar de devolver chaves.
...
* Foi criado o cmdlet "Update-AzApplicationInsights”
* Foram criados cmdlets para a Conta de Armazenamento Associada

#### <a name="azbatch"></a>Az.Batch
* Az.Batch foi atualizado para utilizar a versão do SDK "Microsoft.Azure.Batc" 13.0.0 e a versão do SDK "Microsoft.Azure.Management.Batch" 9.0.0.
* Foi adicionada a capacidade de selecionar o tipo de certificado que vai ser adicionado mediante a utilização do novo parâmetro "-CertificateKind" para "New-AzBatchCertificate".
...
* Agora, ao utilizar "New-AzBatchPool" para criar um conjunto, a propriedade "VirtualMachineImageId" de "PSImageReference" só pode fazer referência a uma imagem do Shared Image Gallery.
* Ao utilizar "New-AzBatchPool" para criar um conjunto, o conjunto pode ser aprovisionado sem um IP público mediante a utilização da nova propriedade "PublicIPAddressConfiguration" de "PSNetworkConfiguration".
  - A propriedade "PublicIPs" de "PSNetworkConfiguration" foi igualmente movida para "PSPublicIPAddressConfiguration". Esta propriedade só pode ser especificada se "IPAddressProvisioningType" for "UserManaged".

#### <a name="azcompute"></a>Az.Compute
* Foi adicionado o parâmetro HostId ao cmdlet "Update-AzVM"
* Os documentos de Ajuda para os cmdlets "New-AzVMConfig", "New-AzVmssConfig", "Update-AzVmss", "Set-AzVMOperatingSystem" e "Set-AzVmssOsProfile" foram atualizados.
...
* A descrição do cmdlet "Get-AzVM" foi atualizada para o descrever melhor. 

#### <a name="azdatafactory"></a>Az.DataFactory
* CRUD suportado das propriedades de fluxo de dados no IR Gerido.

#### <a name="azfrontdoor"></a>Az.FrontDoor
* Foram adicionados cmdlets novos para criação, atualização, obtenção e eliminação do objeto de Motor de Regras do Front Door
* Foram adicionados cmdlets de programas auxiliares para construção do objeto de Motor de Regras do Front Door
* Foi adicionada a referência ao Motor de Regras ao objeto Regra de Encaminhamento do Front Door.
* Foram adicionados parâmetros do Private Link ao objeto de Back-end do Front Door

#### <a name="azfunctions"></a>Az.Functions
* Disponibilidade geral do módulo Az.Functions

#### <a name="azhdinsight"></a>Az.HDInsight
* Encriptação de discos de chave gerida pelo cliente suportada

#### <a name="azhealthcareapis"></a>Az.HealthcareApis
* As políticas de acesso já não são predefinidas para o principal atual

#### <a name="aziothub"></a>Az.IotHub
* Foi adicionado um cmdlet para invocar uma consulta num hub IoT para obter informações mediante a utilização de uma linguagem tipo SQL.
* Foi corrigido um problema em que "Add-AzIotHubDevice" falha ao criar um Dispositivo Compatível com o Edge sem dispositivos subordinados [#11597]
* Foi adicionado o cmdlet para gerar o token de SAS para o Hub IoT, um dispositivo ou um módulo.
* Foi adicionado um cmdlet para invocar a consulta de métricas de configuração.
* Gerir implementações automáticas do IoT Edge em escala. Os novos cmdlets são:
    - "Add-AzIotHubDeployment"
    - "Get-AzIotHubDeployment"
    - "Remove-AzIotHubDeployment"
    - "Set-AzIotHubDeployment"
* Foi adicionado um cmdlet para invocar uma consulta de métricas de implementação do IoT Edge.
* Foi adicionado um cmdlet para aplicar o conteúdo da configuração ao dispositivo edge especificado.

#### <a name="azkeyvault"></a>Az.KeyVault
* Foram removidos dois aliases: "New-AzKeyVaultCertificateAdministratorDetails" e "New-AzKeyVaultCertificateOrganizationDetails"
* A eliminação recuperável foi ativada por predefinição ao criar um cofre de chaves
* Podem ser definidas regras de rede para governar a acessibilidade a partir de localizações na rede ao criar um cofre de chaves
* Foi adicionado suporte para Bring Your Own Key (BYOK)
    - "Add-AzKeyVaultKey" suporta a geração de chaves de intercâmbio de chaves
    - "Get-AzKeyVaultKey" suporta a transferência de chaves públicas no formato PEM
* "KeyOps" foi atualizado como parte do documento de ajuda de "Add-AzKeyVaultKey"

#### <a name="azmonitor"></a>Az.Monitor
* Foi corrigido um erro em "Set-AzDiagnosticSettings"; a política de retenção não se aplicará a todas as categorias [#11589]
* Suporte para critérios de disponibilidade de WebTest para alertas de métricas V2
    - Novo "New-AzMetricAlertRuleV2Criteria": foi adicionada uma opção para criar critérios de disponibilidade WebTest
    - "Add-AzMetricAlertRuleV2": suporta os novos critérios de disponibilidade de WebTest
* Foi removida a definição redundante de RetentionPolicy em PSLogProfile [#7608]
* Foram removidas as propriedades redundantes definidas em PSEventData [#11353]
* O nome de "Get-AzLog" foi mudado para "Get-AzActivityLog"

#### <a name="aznetwork"></a>Az.Network
* Foi adicionado um atributo de alteração interruptiva para notificar que o comportamento predefinido da Zona vai ser alterado
    - "New-AzPublicIpAddress"
    - "New-AzPublicIpPrefix"
    - "New-AzLoadBalancerFrontendIpConfig"
* Foi adicionado suportado para um recurso de nível superior novo SecurityPartnerProvider
...
* Foi adicionado "RequiredZoneNames" em "PSPrivateLinkResource" e "GroupId" em "PSPrivateEndpointConnection”
* Foi corrigido o tipo incorreto do parâmetro SuccessThresholdRoundTripTimeMs para New-AzNetworkWatcherConnectionMonitorTestConfigurationObject
* Foram atualizados os cmdlets VirtualWan para definir o valor predefinido do argumento AllowVnetToVnetTraffic como Verdadeiro.
    - "New-AzVirtualWan"
    - "Update-AzVirtualWan"
* Foram adicionados cmdlets para suportar o grupo de zonas DNS para pontos finais privados
    - "New-AzPrivateDnsZoneConfig"
    - "Get-AzPrivateDnsZoneGroup"
    - "New-AzPrivateDnsZoneGroup"
    - "Set-AzPrivateDnsZoneGroup"
    - "Remove-AzPrivateDnsZoneGroup"
* Foram adicionados os parâmetros "DNSEnableProxy", "DNSRequireProxyForNetworkRules" e "DNSServers" a "AzureFirewall"
* Foram adicionados os parâmetros "EnableDnsProxy", "DnsProxyNotRequiredForNetworkRule" e "DnsServer" a "AzureFirewall"
    - Cmdlet atualizado:
        - New-AzFirewall

#### <a name="azoperationalinsights"></a>Az.OperationalInsights
* O código legado foi atualizado para aplicar o SDK recém-gerado
* Foram eliminados cmdlets devido a APIs preteridas
...
* Foram adicionados parâmetros para "Set-AzOperationalInsightsWorkspace" e "New-AzOperationalInsightsWorkspace"
* Foram criados cmdlets para a Conta de Armazenamento Associada
* Foram criados cmdlets para Clusters e para o Serviço Associado

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* O Azure Site Recovery adicionou suporte para proteger máquinas virtuais de grupo de colocação por proximidade para fornecedor do Azure para o Azure.
* O Azure Site Recovery adicionou suporte para a replicação de zona para zona.
* O Azure Backup adicionou suporte para a retenção de longo prazo para Pontos de Recuperação de Partilhas de Ficheiros do Azure.
* O Azure Backup adicionou propriedades de exclusão de discos à saída do cmdlet "Get-AzRecoveryServicesBackupItem".
* Foi adicionado o ponto final privado para o ficheiro de credenciais de Cofre para o Azure Site Recovery.

#### <a name="azresources"></a>Az.Resources
* Foi adicionado um aviso de mensagem relativo a um atraso na vista ao criar uma Definição de Função nova
* Os cmdlets de política foram alterados para produzir objetos de tipos compatíveis
* Foi removido o parâmetro "-TenantLevel" utilizado no cmdlet "Get-AzResourceLock" [#11335]
* "Remove-AzResourceGroup -Id ResourceId" foi corrigido [#9882]
* Foi adicionado um cmdlet novo para obter resultados de What-If de modelos do ARM no âmbito de um grupo de recursos: "Get-AzDeploymentResourceGroupWhatIfResult"
* Foi adicionado um cmdlet novo para obter resultados de What-If de modelos do ARM no âmbito de uma subscrição: "Get-AzDeploymentWhatIfResult"
   - Alias: "Get-AzSubscriptionDeploymentWhatIf"
* Os parâmetros "-WhatIf" e "-Confirm" foram substituídos para "New-AzDeployment" e "New-AzResourceGroupDeployment" para utilizar resultados de What-If de modelos do ARM
* Foi adicionada a mensagem de descontinuação para o parâmetro "ApiVersion" em cmdlets implementados
* Foi adicionada a capacidade para mostrar mensagens de erro melhoradas relativas a falhas em implementações
* Foi adicionado o registo de correlationId para falhas em implementações
* Foi adicionada a propriedade "error" à saída do script de implementação
* O nuget Microsoft.Azure.Management.ResourceManager foi atualizado para "3.7.1-preview"
* Foram removidos casos de teste específicos, uma vez que a propriedade Error em DeploymentValidateResult foi alterada para apenas de leitura a partir de 3.7.1-preview
* GenericResourceExpanded foi importado do SDK ResourceManager 3.7.1-preview
* Foi adicionado suporte para etiquetas a todos os cmdlets Get para implementação, bem como
    - "New-AzDeployment"
...
#### <a name="azservicefabric"></a>Az.ServiceFabric
* Foi corrigido um erro na adição de certificados mediante a utilização de --SecretIdentifier que obtinha o thumbprint de certificado incorreto

#### <a name="azsql"></a>Az.Sql
* Foi melhorado o desempenho de:
...
* Foi removida a validação do lado do cliente do parâmetro "RetentionDays" do cmdlet "Set-AzSqlDatabaseBackupShortTermRetentionPolicy"
* Auditoria a uma conta de armazenamento na Vnet; corrigido um erro ao criar uma função de Contribuidor de Dados de Blobs de Armazenamento.

#### <a name="azstorage"></a>Az.Storage
* Foi adicionado "-AsJob" ao cmdlet para obter/listar contas "Get-AzStorageAccount"
* KeyVersion foi tornado opcional ao atualizar a conta de Armazenamento com KeyvaultEncryption, para suportar a rotação automática de chaves
    - "Set-AzStorageAccount"
* Corrigida a remoção de falha do Diretório de Ficheiros do Azure com o pipeline
    - "Remove-AzStorageDirectory"
* Corrigido [#9880]: A definição do valor DefaultAction de NetWorkRule foi alterado para estar alinhado com o swagger.
    - "Update-AzStorageAccountNetworkRuleSet"
    - "Get-AzStorageAccountNetworkRuleSet"
* Corrigido [#11624]: Ignorar regras duplicadas ao adicionar NetworkRules, para evitar falhas de servidores
    - "Add-AzStorageAccountNetworkRule"
* O SDK Microsoft.Azure.Cosmos.Table foi atualizado para 1.0.7
* Foi adicionada uma mensagem de aviso para relembrar o utilizador para listar novamente com ContinuationToken apenas quando são devolvidos itens de peças na lista DataLake Gen2 Items,
    - "Get-AzDataLakeGen2ChildItem"
* Suportado para criar ou atualizar a conta de Armazenamento com a Autenticação do Active Directory Domain Service dos Ficheiros do Azure
    -  "New-AzStorageAccount"
    -  "Set-AzStorageAccount"
* Suportado para os comandos "new" ou "list" de chaves de Kerberos da conta de Armazenamento
    -  "New-AzStorageAccountKey"
    -  "Get-AzStorageAccountKey"
* Suportada conta de Armazenamento de ativação pós-falha
...
* Integração da biblioteca de cliente do Armazenamento v12 para cmdlets de Filas e Ficheiros
* Alteração ao tipo de saída de CloudFile para AzureStorageFile; a saída original tornar-se-á na propriedade subordinada da saída nova
    - "Get-AzStorageFile"
    - "Remove-AzStorageFile"
    - "Get-AzStorageFileContent"
    - "Set-AzStorageFileContent"
    - "Start-AzStorageFileCopy"
* Alteração ao tipo de saída de CloudFileDirectory para AzureStorageFileDirectory; a saída original tornar-se-á na propriedade subordinada da saída nova
    - "New-AzStorageDirectory"
    - "Remove-AzStorageDirectory"
* Alteração ao tipo de saída de CloudFileShare para AzureStorageFileShare; a saída original tornar-se-á na propriedade subordinada da saída nova
    - "Get-AzStorageShare"
    - "New-AzStorageShare"
    - "Remove-AzStorageShare"
* Alteração ao tipo de saída de FileShareProperties para AzureStorageFileShare; a saída original tornar-se-á na propriedade subordinada da saída nova
    - "Set-AzStorageShareQuota"

#### <a name="aztrafficmanager"></a>Az.TrafficManager
* Corrigido o nome de perfil errado na saída verbosa de "DisableAzureTrafficManagerEndpoint"

#### <a name="azwebsites"></a>Az.Websites
* Corrigido um erro ortográfico na ajuda de "Update-AzWebAppAccessRestrictionConfig".

## <a name="380---april-2020"></a>3.8.0 - Abril de 2020
### <a name="highlights-since-the-last-release"></a>Destaques desde a última versão
* Versões do Powershell suportadas pelo Az.Storage: Windows PowerShell 5.1, PowerShell Core 6.2.4+, PowerShell 7
...
## <a name="0100-preview---april-2020"></a>0.10.0-preview - Abril de 2020
### <a name="general"></a>Geral
* Os módulos Az já estão disponíveis em pré-visualização no Azure Stack Hub. Isto permite a compatibilidade entre plataformas diferentes com o Linux e macOS. Agora, o Azure Stack Hub suporta o PowerShell Core com os módulos Az. Obtenha mais informações [aqui](https://aka.ms/az4AzureStack)
* Os módulos Az suportam o perfil 2019-03-01-hybrid:
  - Az.Billing
  - Az.Compute
  - Az.DataBoxEdge
  - Az.EventHub
  ...
  - Az.Storage
  - Az.Websites
* Foram introduzidos três novos módulos do PowerShell para Az que funcionam com o Azure Stack Hub. São eles o Az.Databox, Az.IotHub e Az.EventHub
* Os comandos permanecem sensivelmente iguais, com pequenas alterações como, por exemplo, a alteração do AzureRM para Az
* Veja uma ligação atualizada à documentação do PowerShell para o Azure Stack Hub [aqui](https://aka.ms/InstallASHPowerShell)
